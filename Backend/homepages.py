...
@app.post("/burger/change_profile")
async def change_profile_post(
    ...
    if not validate_name(name) or not validate_date_format(dob) or not validate_email(email):
        return RedirectResponse(url=f"/burger/change_profile?name={name}&dob={dob}&email={email}", status_code=302)
...
async def go_back_article():
    return RedirectResponse(url="/", status_code=302)

@app.get("/article/go_to_external_link")
async def go_to_external_link():
    return RedirectResponse(url=article_link, status_code=302)
